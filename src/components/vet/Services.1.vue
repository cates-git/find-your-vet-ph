<template>
    <v-card class="elevation-0 transparent text-xs-left  mt-2 mb-4 mx-4">
        ...
        <p class="blue--text text-xs-center" v-if="list.length === 0 && !loading">No services.</p>
        <v-container
            fluid
            grid-list-md
            class="pa-0">
            <v-layout 
                ...
                <v-flex v-for="(item, i) in list" :key="i" sm6 md4>
                    <v-card>
                        ...
                        <v-card-actions v-if="$store.getters.USER_DATA.type == 1">
                            <v-btn color="primary" @click="updateService(item)" flat style="opacity: 1">
                                Edit
                            </v-btn>
                            <v-btn @click="confirmDelete(item.id)" color="error" flat style="opacity: 1">
                                Delete
                            </v-btn>
                        </v-card-actions>
                    </v-card>
...
export default {
	...
	methods: {
		...
		confirmDelete (id) {
			this.deleteId = id
			this.$store.dispatch('SHOW_CONFIRM', 'Service will be deleted. Continue ?')
		},
        continueDelete () {
            if (!this.deleteId) {
