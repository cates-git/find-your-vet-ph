<template>
    <v-card class="elevation-0 transparent text-xs-left  mt-2 mb-4 mx-4" v-if="$store.getters.USER_DATA.type == $userTypes.ADMIN">
        <v-card-title primary-title class="px-0">
            <div>
                <h3 class="headline mb-0">Veterinarians</h3>
            </div>
        </v-card-title>
...
export default {
    ...
    methods: {
		...
		confirmDelete (id) {
			this.deleteId = id
			this.$store.dispatch('SHOW_CONFIRM', 'Veterinarian and everything that is linked to the user will be deleted permanently and cannot be undone. Continue ?')
		},
        continueDelete () {
            if (!this.deleteId) {
